...
public class Suite implements TestClassFilter {
	...
	private static IMongodConfig mongodConfig() throws IOException {
		return new MongodConfigBuilder().version(version()).net(new Net(getPort(), Network.localhostIsIPv6())).replication(
				new Storage(storagePath, null, 0)).timeout(new Timeout()).build();
	}
	...
	private static MongodExecutable mongodExecutable() throws IOException {
	 ...
	 *
	 * @param suite
	 *            started from suite or not, tells if mongo will be started/stopped actually
	 */
