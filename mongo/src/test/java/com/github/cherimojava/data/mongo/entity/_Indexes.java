    ...
    extends MongoBase
{
    ...
    public void indexCreation()
    {
        ...
        int count = 0;
        for ( Document document : db.getCollection( EntityUtils.getCollectionName( IndexedEntity.class ) )
            .listIndexes() )
        {
            count++;
            assertThat( document.toJson().replaceAll( "\"", "" ),
                anyOf( sameJSONAs( "{v:1, key:{_id:1}, name:_id_, ns:_Indexes.collection}" ),
                    sameJSONAs(
                        "{v:1, key:{string:1}, name:single, ns:_Indexes.collection}" ),
                sameJSONAs(
                    "{v:1, unique:true, key:{string:-1, anotherString:1}, name:string_-1_anotherString_1, ns:_Indexes.collection}" ) ) );
    ...
    @Named( "collection" )
    private interface IndexedEntity
        extends Entity<IndexedEntity>
    {
    ...
    @Collection( indexes = {
        @Index( name = "single", value = { @IndexField( field = "string", order = IndexField.Ordering.ASC ) }) })
    private interface InvalidIndexedEntity
        extends Entity<InvalidIndexedEntity>
    {
    }
}
